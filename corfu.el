...
with the result of `truncate-string-ellipsis'."
  (let* ((face (get-text-property 0 'face str))
    ...
    (while (and glyphs (<= glyph-width width))
      ...
      (setq glyph-width (string-pixel-width (car glyphs))))

    (when glyphs
      (while (> ellipsis-width width)
        (setq glyphs (cons (car result) glyphs))
        (setq result (cdr result))
        (setq width (+ width glyph-width))
        (setq glyph-width (string-pixel-width (car result))))
...
CURR is the currently selected candidate."
  (let* ((cw (cl-loop for x in cands maximize (string-pixel-width (car x))))
         ...
         (width (+ pw cw sw))
         (min-width (* corfu-min-width (default-font-width)))
         ;; -4 because of margins and some additional safety
         (line-width (* (min corfu-max-width (- (frame-width) 4)) (default-font-width))))
    ...
    (cl-loop for (cand prefix suffix) in cands
             with prefix-face = nil
             with cand-face = nil
             with suffix-face = nil
             with i = 0
             do
             ...
             (setq prefix-face (get-text-property 0 'face prefix)
                   cand-face (get-text-property 0 'face cand)
                   suffix-face (get-text-property 0 'face suffix)
                   cand (corfu--truncate-string-to-pixel-width cand cw))
             ...
             (concat
              prefix
              (apply 'propertize " " 'display `(space :align-to (,pw)) (if prefix-face `(face ,prefix-face)))
              cand
              (apply 'propertize " " 'display `(space :align-to (,cw)) (if cand-face `(face ,cand-face)))
              (when (> sw 0)
                (apply 'propertize " " 'display `(space :align-to (,(+ pw cw sw (- (string-pixel-width suffix)))))
                       (if suffix-face `(face ,suffix-face))))
              (when (> sw 0)
                suffix)))))
...
(cl-defgeneric corfu--decorate (cands)
...
(define-obsolete-function-alias 'corfu--affixate 'corfu--decorate "1.6")
    ...
    (cond
     ...
     ((pcase-exhaustive corfu-quit-no-match
      ...
      (corfu--popup-show (posn-at-point beg) 0 (string-pixel-width no-match-msg)
                         (list (propertize no-match-msg 'face 'italic))))
